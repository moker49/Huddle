...
drop function if exists public.create_huddle(text, timestamptz, jsonb);
...
begin
  ...
  insert into public.huddles (owner_id, title, icon, auto_archive_at)
  values (auth.uid(), trim(p_title), nullif(trim(p_icon), ''), p_auto_archive_at)
...
grant execute on function public.create_huddle(text, text, timestamptz, jsonb) to authenticated;

drop function if exists public.update_huddle(uuid, text, timestamptz, jsonb);
...
begin
  ...
  set title = trim(p_title), icon = nullif(trim(p_icon), ''), auto_archive_at = p_auto_archive_at
  where huddle.id = p_huddle_id;
...
grant execute on function public.update_huddle(uuid, text, text, timestamptz, jsonb) to authenticated;
