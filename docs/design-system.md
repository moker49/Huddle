...
Editable text fields should provide an in-field clear action when they have a value. This keeps correction fast on mobile and avoids requiring long-press selection or repeated backspacing. Clear actions must preserve the current focus state: if the field is focused, clearing should leave it focused; if it is not focused, clearing should not focus it. Use press-start focus preservation for custom clear buttons and disable Paper's forced input focus on `TextInput.Icon` clear buttons. The current exceptions are intentional: the main search field keeps its existing clear/profile trailing action, and the profile display-name field reserves its trailing adornment area for tag and phone identity actions rather than a clear button.
